import { DndContext, DragEndEvent, MeasuringStrategy, PointerSensor, closestCenter, useSensor, useSensors } from "@dnd-kit/core";
...
import { ImageBlockElement, ImageItemWithUpload } from "./types";
...
interface Props {
  ...
  onCaptionChange: (item: ImageItemWithUpload | null, val: string | null) => void;
...
export const ImageBlockGrid: FC<Props> = ({
  ...
  return (
    <div className="flex flex-col gap-1">
              ...
              {items.map((item, idx) => (
                <ImageDraggableItem
                  ...
                  className={clsx([
                    ...
                    items.length === 3 ? idx === 0 ? "row-span-2" : "col-span-1 row-span-1" : "",
      ...
      {items.map((item, idx) => (items.length == 1 && idx == 0) || focus?.id === item.id ? (
        <Caption
