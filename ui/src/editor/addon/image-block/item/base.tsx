...
export const ImageBaseItem = memo(forwardRef<HTMLDivElement, ImageBaseItemProps>(({
  isDragging,
  className,
  item,
  onUploadComplete,
  ...
  const url = useMemo(
    () => localUrl
      || item.url, // In Simian, url stores the full relative path
    [localUrl, item.url]
  );

  const handleUpload = useCallback(async () => {
    if (!item.upload || item.upload.item) { return; }
  ...
  const imageView = (
    ...
    >
      ...
      <div
        className="absolute inset-0 p-4 z-3 w-full h-full flex flex-col"
      >
