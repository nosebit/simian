...
const insertText: LatexInlineAddon["insertText"] = ({
  ...
  if (text == "$" && selection?.isCollapsed) {
    ...
    if (match) {
      ...
      const startPoint = Editor.before(editor, anchor, { distance: matchLength });
 ...
 */
const onChange: LatexInlineAddon["onChange"] = ({
  editor,
  selection,
}) => {
  const resnapPoint = (point: Point, isFocusPoint: boolean) => {
    ...
    const [match] = Editor.nodes(editor, {
      at: point,
      match: (n) => Element.isElement(n) && n.type === "latex-inline" && n.mode === "read",
    });
