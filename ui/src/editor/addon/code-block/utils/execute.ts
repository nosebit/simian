  ...
  try {
    ...
    while (true) {
      ...
      const isolatedOutput =
        parts.length > 1 ? parts.slice(1).join(marker) : rawOutput

      // Handle carriage returns for in-place progress bar updates
      // eslint-disable-next-line no-control-regex
      const cleanOutput = isolatedOutput.replace(/\x1b\[[0-9;]*[a-zA-Z]/g, '') // strip ANSI codes
      ...
      while (
        ...
        if (endIdx !== -1) {
          ...
          tempOutput =
            tempOutput.substring(0, startIdx) + tempOutput.substring(afterEnd)
        } else {
          break
        }
      }
      ...
      Transforms.setNodes(
        editor,
        {
          isEvaluating: true,
          output: {
            stdout: success ? processedOutput : '',
            stderr: success ? '' : processedOutput,
            success,
            items,
          },
        },
