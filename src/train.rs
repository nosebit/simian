...
pub fn fit<M: Model, O: Optimizer, L: Loss>(
  ...
  for epoch in 1..=epochs {
    ...
    if epoch % (epochs.max(20) / 20) == 0 || epoch == epochs || epoch == 1 {
      let current_msg = format!("Epoch {:5} | Train Loss: {:.4} | Val Loss: {:.4}", epoch, avg_train_loss, val_loss);
      ...
      if studio_mode {
        ...
        let pb_str = format!("\r[{}{}] {}/{} {}", progress_chars, space_chars, epoch, epochs, last_two_msgs.replace("\n", " || "));
        eprint!("{}", pb_str);
      } else {
