...
use cli::{
  Cli, Commands, DataCommands, ModelCvCommands, ModelEvalCommands, ModelFitCommands, PrepCommands,
  SplitCommands, PaperCommands,
};
...
use simian::model::{
  algo::{LinearRegression, LogisticRegression, PolynomialRegression}, Model,
};
...
fn resolve_out_model_path(
  ...
  let file_stem = relative_path.file_stem().unwrap_or_default().to_string_lossy().to_string();
  let parent = relative_path.parent().unwrap_or_else(|| std::path::Path::new(""));

  let models_dir = get_models_dir()?;
  let out_path = models_dir.join(parent).join(format!("{}.{}.st", file_stem, suffix));
    ...
    N: for<'a> FormatFields<'a> + 'static,
{
    fn format_event(
        ...
        let mut visitor = MessageVisitor { writer: writer.by_ref(), result: Ok(()) };
  ...
  match &cli.command {
    ...
    Commands::Data { command } => match command {
      ...
      DataCommands::Prep { preparer } => match preparer {
        ...
        PrepCommands::Polynomial { degree, dataset, out } => {
          let in_path = resolve_in_path(&dataset)?;
    ...
    Commands::Fit { model } => match model {
      ModelFitCommands::Linear { args } => {
        ...
        let all_cols: Vec<String> = train_data.df.get_column_names().into_iter().map(|s| s.to_string()).collect();
        let target_col = args.target.clone().unwrap_or_else(|| all_cols.last().unwrap().to_string());
        ...
        linear_model.fit(&train_data, &train_data, &target_col, &device)
          .learning_rate(args.learning_rate)
          .epochs(args.epochs)
          .batch_size(args.batch_size)
          .patience(args.patience)
          .tolerance(args.tolerance)
          .log_path(Some(std::path::Path::new(&train_path).with_extension("log").to_string_lossy().to_string()))
          .run()?;
      ...
      ModelFitCommands::Polynomial { degree, args } => {
        ...
        poly_model.fit(&train_data, &train_data, &target_col, &device)
          .learning_rate(args.learning_rate)
          .epochs(args.epochs)
          .batch_size(args.batch_size)
          .patience(args.patience)
      ...
      ModelFitCommands::Logistic { args } => {
        ...
        logistic_model.fit(&train_data, &train_data, &target_col, &device)
          .learning_rate(args.learning_rate)
          .epochs(args.epochs)
          .batch_size(args.batch_size)
          .patience(args.patience)
    ...
    Commands::Eval { model } => match model {
      ModelEvalCommands::Score { args } => {
        ...
        let predictions = if model_path.ends_with(".polynomial.st") {
          ...
          anyhow::bail!("Cannot infer model type from filename '{}'. Ensure the model file ends with .linear.st, .polynomial.st, or .logistic.st", model_path);
        };
      ...
      ModelEvalCommands::Plot {
        ...
        let html = if model_path.ends_with(".polynomial.st") {
          let poly_model = PolynomialRegression::load(&model_path, &device)?;
          poly_model.plot(&test_data, &feature_cols, &target_col, &device).run()?
        } else if model_path.ends_with(".linear.st") {
          let lin_model = LinearRegression::load(&model_path, &device)?;
          lin_model.plot(&test_data, &feature_cols, &target_col, &device).run()?
        } else if model_path.ends_with(".logistic.st") {
          let log_model = LogisticRegression::load(&model_path, &device)?;
          log_model.plot(&test_data, &feature_cols, &target_col, &device).run()?
        } else {
          anyhow::bail!("Cannot infer model type from filename '{}'. Ensure the model file ends with .linear.st, .polynomial.st, or .logistic.st", model_path);
        };
    ...
    Commands::Cv { model } => match model {
      ModelCvCommands::Linear { args } => {
        ...
        let all_cols: Vec<String> = dataset_obj.df.get_column_names().into_iter().map(|s| s.to_string()).collect();
        let target_col = args.target.clone().unwrap_or_else(|| all_cols.last().unwrap().to_string());
        ...
        let (avg_mse, avg_r2) = simian::eval::cv_regression(
          ...
          |train_data| {
            ...
            linear_model.fit(train_data, train_data, &target_col, &device)
              ...
              .patience(args.patience)
              .tolerance(args.tolerance)
              .log_path(Some(std::path::Path::new(&dataset_path).with_extension("log").to_string_lossy().to_string()))
              .run()?;
            Ok(linear_model)
          },
          &target_col,
