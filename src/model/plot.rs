...
impl<'a, M: Model> ModelPlotJob<'a, M> {
  pub fn new(dataset: &'a Dataset, model: &'a M, x_cols: &[&'a str], y_col: &'a str, device: &'a Device) -> Self {
    Self {
      dataset,
      model,
  ...
  pub fn run(self) -> Result<String> {
    if self.x_cols.is_empty() || self.x_cols.len() > 2 {
      bail!("ModelPlotJob requires 1 or 2 features in x_cols, found {}", self.x_cols.len());
    }

    let default_title = if let Some(t) = &self.title {
      t.clone()
    } else {
      format!("Model Fit ({} vs {})", self.y_col, self.x_cols.join(" and "))
    };
    ...
    let y_pred: Vec<f32> = predictions.to_vec2::<f32>()?.into_iter().map(|v| v[0]).collect();

    let plot = if self.x_cols.len() == 1 {
      let x_col = get_f32_col(self.x_cols[0])?;
      let mut pairs: Vec<(f32, f32)> = x_col.clone().into_iter().zip(y_pred.into_iter()).collect();
      pairs.sort_by(|a, b| a.0.partial_cmp(&b.0).unwrap());
      let x_line: Vec<f32> = pairs.iter().map(|p| p.0).collect();
      let y_line: Vec<f32> = pairs.iter().map(|p| p.1).collect();

      build_model_fit_2d(
        x_col, target_col, x_line, y_line, self.x_cols[0], self.y_col, &default_title
      )
    } else {
      let x_col = get_f32_col(self.x_cols[0])?;
      let y_col = get_f32_col(self.x_cols[1])?;
      build_model_fit_3d(
        x_col.clone(), y_col.clone(), target_col, x_col, y_col, y_pred, self.x_cols[0], self.x_cols[1], self.y_col, &default_title
      )
    };
