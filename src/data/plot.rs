...
impl<'a> DatasetPlotJob<'a> {
  ...
  pub fn run(self) -> Result<String> {
    if self.x_cols.is_empty() || self.x_cols.len() > 2 {
      bail!("PlotJob requires 1 or 2 features in x_cols, found {}", self.x_cols.len());
    }
    ...
    let plot = if self.x_cols.len() == 1 {
      ...
      build_2d_scatter(x_col, target_col, self.x_cols[0], self.y_col, &default_title)
    } else {
      let x_col = get_f32_col(self.x_cols[0])?;
      let y_col = get_f32_col(self.x_cols[1])?;
      build_3d_scatter(x_col, y_col, target_col, self.x_cols[0], self.x_cols[1], self.y_col, &default_title)
    };
