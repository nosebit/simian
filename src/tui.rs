...
pub fn run_dataset_tui(mut lf: LazyFrame) -> Result<()> {
  ...
  // Run the TUI loop
  let res = run_loop(&mut terminal, &mut lf, &column_names, &mut offset, page_size);
  ...
  loop {
    ...
    terminal.draw(|f| {
      ...
      let t = Table::new(rows, constraints)
        .header(header)
        .block(Block::default().borders(Borders::ALL).title("Simian Data Explorer"))
      ...
      let info = Paragraph::new(Line::from(vec![
        Span::raw(format!(" Rows {}-{} ", *offset, *offset + current_chunk.height() as i64)),
        Span::styled(" | Use Up/Down/PageUp/PageDown to scroll | 'q' to quit ", Style::default().fg(Color::Cyan)),
      ]))
