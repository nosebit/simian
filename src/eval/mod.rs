...
pub fn f1_score(predictions: &Tensor, targets: &Tensor) -> Result<f32> {
  ...
  for i in 0..total {
    ...
    if pred == 1.0 && truth == 1.0 { tp += 1.0; }
    if pred == 1.0 && truth == 0.0 { fp += 1.0; }
    if pred == 0.0 && truth == 1.0 { fn_val += 1.0; }
  }
  ...
  M: Model,
{
  ...
  for (i, (train_data, test_data)) in folds.iter().enumerate() {
    ...
    tracing::info!("Fold {}: Accuracy = {:.4}, F1 = {:.4}", i + 1, fold_acc, fold_f1);
...
mod tests {
  ...
  fn test_r2_score() -> Result<()> {
    let device = Device::Cpu;
    let y_pred = Tensor::new(&[[2.5f32], [0.0], [2.0], [8.0]], &device.as_candle().unwrap())?;
    let y_true = Tensor::new(&[[3.0f32], [-0.5], [2.0], [7.0]], &device.as_candle().unwrap())?;
