...
async fn execute_code(State(state): State<Arc<AppState>>, Json(payload): Json<ExecuteRequest>) -> Response {
...
async fn get_content(State(state): State<Arc<AppState>>) -> Json<serde_json::Value> {
  if state.studio_file.exists() {
    if let Ok(content) = std::fs::read_to_string(&state.studio_file) {
...
async fn serve_html(axum::extract::Query(query): axum::extract::Query<HtmlQuery>) -> axum::response::Html<String> {
...
fn resolve_studio_path(path: Option<String>) -> anyhow::Result<PathBuf> {
  ...
  if !absolute.exists() {
    ...
    let _ = std::fs::write(&absolute, serde_json::to_string_pretty(&default_content).unwrap());
  }
