...
pub async fn run_rust_code(code: &str, studio_file: &std::path::Path) -> Result<tokio::process::Child> {
  ...
  let proj_path = home.join(".simian").join("sandboxes").join(format!("sandbox_{}", hash));
  ...
  cargo_toml.push_str("polars = { version = \"0.41\", features = [\"csv\", \"parquet\", \"lazy\", \"describe\"] }\n");
