...
pub fn render(format: RenderFormat, content: &str) {
    let final_url = match format {
        RenderFormat::Html => {
            ...
            format!("data:text/html;base64,{}", general_purpose::STANDARD.encode(content))
        },
        RenderFormat::Url => content.to_string(),
    };
    
    println!("__SIMIAN_BEGIN_CONTENT\n{}\n__SIMIAN_END_CONTENT", final_url);
}
...
async fn authenticate(client: &Client) -> Result<String> {
    ...
    if auth_file.exists() {
        if let Ok(content) = std::fs::read_to_string(&auth_file) {
            if let Ok(stored) = serde_json::from_str::<StoredAuth>(&content) {
    ...
    loop {
        ...
        if let Some(token) = token_res.access_token {
            ...
            let stored = StoredAuth { access_token: token.clone() };
...
fn get_paper_dir(id: &str) -> Result<PathBuf> {
    ...
    anyhow::bail!("Paper '{}' not found in current directory or ~/.simian/papers/", id);
}
...
pub async fn submit(id: String) -> Result<()> {
    ...
    tracing::info!("Authentication successful. Token begins with: {}...", &token[..4]);
    ...
    if let Some(blocks) = ast.as_array() {
        for block in blocks {
            ...
            if block_type == "paragraph" || block_type == "text" {
                ...
                let lang = block.get("language").and_then(|v| v.as_str()).unwrap_or("rust");
                markdown.push_str(&format!("```{}\n{}\n```\n", lang, code));

                // Find plots in output
                if let Some(output) = block.get("output") {
                    if let Some(urls) = output.get("urls").and_then(|v| v.as_array()) {
                        for url_val in urls {
                            if let Some(url) = url_val.as_str() {
                                if url.starts_with("data:text/html;base64,") {
                                    ...
                                    if parts.len() == 2 {
                                        ...
                                        if let Ok(decoded) = general_purpose::STANDARD.decode(b64) {
                                            let rand_id = rand::random::<u32>();
                                            let ext = if url.starts_with("data:image/png") { "png" } else { "jpg" };
